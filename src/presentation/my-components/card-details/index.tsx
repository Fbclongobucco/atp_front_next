...
export function CardDetails({ player, isOpen, onClose }: CardDetailsProps) {
    ...
    return (
        ...
        backdrop-blur-xl bg-slate-700/50  z-50`}>
            {player && (
                <Card className="w-md lg:w-lg animate__animated animate__backInDown">
                    ...
                    <CardContent className="flex gap-2 lg:gap-10">
                       {player.urlPhoto ===  "https://placehold.co/400x400.png" ? <div className="w-45 h-90 flex justify-center items-center "><User size={200}color="#302e46" /></div> :   <img src={player.urlPhoto} alt="logo" width={200} className="object-cover border rounded  shadow-slate-500 shadow-md bg-slate-300" />}
                        <div className="flex flex-col text-md gap-6 font-medium text-slate-600">
                            <p className="bg-slate-200 rounded p-1">Forehand: <span className="text-md tracking-wide uppercase">{player.forehand}</span></p>
                            <p className="bg-slate-200 rounded p-1">Backhand: <span className="text-md tracking-wide uppercase">{player.backhand}</span></p>
                            ...
                            <p className="bg-slate-200 rounded p-1 flex justify-start items-center">
                                País: <span className="text-md tracking-wide uppercase ml-2">{player.country}</span>
                                <span className="ml-auto"><img src={`https://flagcdn.com/${flagCode}.svg`} width={20} alt="" /></span>
                            </p>
