...
export function CardPlayer({ player }: CardPlayerProps) {
    return (
        <div className="w-[97%] h-20 bg-slate-200 flex gap-1 justify-center items-center p-2 
            ...
            {player.ranking && <p className="mr-auto font-mono font-semibold text-cyan-800 min-w-20">#{player.ranking}</p>}
           
            <div className="w-1/2 flex justify-start items-center">
                {player.urlPhoto === null ? <User className="min-w-[50px]" size={50} color="#302e46" /> : <img src={player.urlPhoto} alt="logo" width={50} height={50} className="object-cover min-w-[50px]" />}
            </div>
            <div className="flex justify-center items-center w-[40%]">
                 <p className="mr-auto font-mono font-semibold text-cyan-800 min-w-10 hidden lg:block">{player.points}</p>
